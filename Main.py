...
class PathFinder:
    ...
    def exec_bfs(self):
        ...
        if found: self.draw_final_path(parent_map, self.target_node)
        else: messagebox.showerror("Error", "No path found via BFS!")
    ...
    def exec_dfs(self):
        ...
        if found: self.draw_final_path(parent_map, self.target_node)
        else: messagebox.showerror("Error", "No path found via DFS!")
    ...
    def exec_ucs(self):
        ...
        if parent_map: self.draw_final_path(parent_map, self.target_node)
        else: messagebox.showerror("Error", "No path found via UCS!")

    def exec_dls(self, limit):
        if not self.check_ready(): return
        parent_map = DLS(self.get_adj(), self.start_node, self.target_node, limit, self.visualize_callback)
        if parent_map: self.draw_final_path(parent_map, self.target_node)
        else: messagebox.showwarning("Cut off", f"Target not found within depth {limit}")

    def exec_idds(self):
        if not self.check_ready(): return
        parent_map = IDDS(self.get_adj(), self.start_node, self.target_node, self.grid_size*2, self.visualize_callback)
        if parent_map: self.draw_final_path(parent_map, self.target_node)
        else: messagebox.showerror("Error", "IDDFS failed to find target.")
        ...
        if res:
            ...
            while curr in b_map and b_map[curr] is not None:
                ...
                if curr != self.target_node:
                    self.visualize_callback(curr, "yellow")
                    time.sleep(0.02)
        else: messagebox.showerror("Error", "Bidirectional search failed.")
